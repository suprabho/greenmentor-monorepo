/**
 * NIC-2008 sector/industry resolution for the orchestrator's grounding tools.
 *
 * NOTE: this module is a per-package COPY of @gm/platform's lib/brsr/nic-sector.ts
 * (same precedent as the per-app lib/msci and lib/nic copies). The platform copy
 * is the source of truth — edit there and re-copy here; ./sector.test.ts pins the
 * behaviour the copy has to keep.
 *
 * The orchestrator needs it because the nic-framework-materiality agent accepts a
 * raw NIC code (2-8 digits, as BRSR filers report it) and has to reduce it to the
 * Division that the SASB / Sustainalytics / MSCI crosswalks are keyed by. Peer
 * tools read division_code straight off brsr_company_activities, so this is only
 * on the code-in path — plus turnoverWeightedSector for the symbol-in path, which
 * weights a filer's product rows into its dominant Divisions.
 *
 * The canonical NIC-2008 tree (21 sections / 88 divisions / 238 groups) lives in
 * community-engine/lib/nic/classification.ts; the two tables below are the
 * Division (2-digit) and Section (letter) rows of it, transcribed verbatim.
 *
