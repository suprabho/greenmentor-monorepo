 ...
 *
 * NOTE: this module is a per-package COPY of @gm/platform's lib/msci/materiality-map.ts
 * (same precedent as the per-app community-engine copy, and as ../../nic/sector.ts).
 * The platform copy is the source of truth — edit there and re-copy here. The
 * orchestrator needs a copy because the MSCI dataset is committed TypeScript, not a
 * Supabase table, so the nic-framework-materiality grounding tool cannot read it the
 * way it reads the SASB / Sustainalytics tables.
 */
