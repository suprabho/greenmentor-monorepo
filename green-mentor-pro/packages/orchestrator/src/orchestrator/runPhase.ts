...
import { loadAgent, runAgent } from "@gm/agents";
import { runCallableTool } from "../toolHandlers";
import { agentsRoot } from "../agentsRoot";
import { getEngagement } from "../db/engagements";
import { getPhaseStates, setPhaseStatus, transitionPhase, cascadeStaleDownstream } from "../db/phases";
import { getArtifactsForPhases, supersedeAndInsert } from "../db/artifacts";
import { createRun, completeRun, failRun } from "../db/runs";
import { openPhaseGate, fanoutFieldReviews, fanoutOpenQuestions } from "../db/reviews";
import { insertValidations } from "../db/validations";
import { PHASE_PRIMARY_ARTIFACT, type Json } from "../db/types";
...
export async function runPhase(
  ...
  const agent = loadAgent(path.join(agentsRoot(), def.agentKey));
