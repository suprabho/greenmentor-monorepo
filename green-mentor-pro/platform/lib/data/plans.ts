 ...
 *
 * Course rows are derived from the live catalog rather than hand-listed, so this
 * block and the course grid on the same page can't disagree. Every course price
 * is a real list price from the intake sheet; only the live-Q&A row is still an
 * estimate.
...
/**
 * Build the value stack from the catalog. Free courses are skipped — a ₹0 row
 * adds nothing to a standalone-value argument. The grouped remainder is an exact
 * sum, not an estimate, so it carries no "est." marker.
 */
export function buildValueStack(courses: { title: string; priceInr: number | null; includedInPlus: boolean }[]): {
  rows: ValueStackRow[];
  total: number;
} {
