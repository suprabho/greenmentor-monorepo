...
const SUGGESTIONS = [
  ...
  "Draft a data request for monthly grid electricity at the Pune plant",
];

export default function BuddyPage() {
  const { messages, sendMessage, status, error } = useChat({
    transport: new DefaultChatTransport({ api: "/api/buddy/chat" }),
  });
  ...
  return (
    <div
      style={{ maxWidth: 760, margin: "0 auto", display: "flex", flexDirection: "column", height: "calc(100vh - 7rem)", color: "#1a2420" }}
    >
      {/* header */}
      <div style={{ display: "flex", alignItems: "center", gap: 11, marginBottom: 14 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: "#5d6b64" }}>ESG &amp; BRSR assistant · same gateway + tools as the agent pipeline (@gm/agents)</div>
        </div>
      </div>
        ...
        {messages.length === 0 ? (
          <div style={{ background: "#fff", border: `1px solid ${BORDER}`, borderRadius: 14, padding: 22, marginTop: 8 }}>
            ...
            <div style={{ fontSize: 13.5, color: "#5d6b64", marginBottom: 16, lineHeight: 1.5 }}>
              I explain frameworks (BRSR, GRI, ISSB, ESRS, TCFD), emissions accounting, and the GreenMentor agent
              pipeline. I won&apos;t invent numbers — for exact factors I point you to the Calculation agent.
            </div>
      ...
      {/* composer */}
      <form onSubmit={(e) => { e.preventDefault(); send(input); }} style={{ paddingTop: 8 }}>
        ...
        <div style={{ fontSize: 11, color: "#9aa6a0", textAlign: "center", marginTop: 7 }}>
          ESG Buddy can be wrong — verify figures with the Calculation agent.
        </div>
      </form>
    </div>
  );
