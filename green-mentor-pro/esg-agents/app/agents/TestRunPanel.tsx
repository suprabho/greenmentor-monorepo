 ...
 * every other agent gets a JSON input box. Both post to the existing
 * /api/agents/[key]/run demo path, which binds the app's callable-tool handlers —
 * so runs here are grounded, not stubbed.
 */
...
export default function TestRunPanel({ agentKey }: { agentKey: string }) {
  ...
  const preview = async () => {
    ...
    setResult(null);
    try {
      const res = await fetch(`/api/brsr/peer-preview?symbol=${encodeURIComponent(selected.symbol)}`);
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error ?? `HTTP ${res.status}`);
      setResult({ data, meta: null, mode: "preview" });
    } catch (e) {
  ...
  const canRun = !busy && (!isPeer || !!selected);
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
        ...
        {busy === "run" && (
          <span style={{ fontSize: 12, color: MUTED }}>grounding + web research can take a minute</span>
        )}
