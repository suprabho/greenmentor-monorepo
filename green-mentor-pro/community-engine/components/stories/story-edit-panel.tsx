...
export function StoryEditPanel({
  ...
  // Serialize the current body to Substack-friendly HTML and put it on the
  // clipboard as text/html, so pasting into the Substack editor keeps the
  // structure (headings, quotes, lists, links, images). Needs a user gesture +
  // secure context (localhost/https) for the async Clipboard API.
  const copyForSubstack = async () => {
    setExporting(true);
    setStatus({ type: "idle" });
    try {
      const res = await fetch(`/api/stories/${story.id}/export`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ body_markdown: bodyMarkdown }),
      });
      const data = (await res.json().catch(() => ({}))) as { html?: string; error?: string; mode?: string };
      if (!res.ok) throw new Error(data.error ?? `HTTP ${res.status}`);
      if (data.mode === "unconfigured") throw new Error("Service-role key isn't configured server-side");
      ...
      setStatus({ type: "ok", msg: "Copied — paste into Substack" });
    } catch (err) {
      setStatus({ type: "err", msg: err instanceof Error ? err.message : "Could not export" });
    } finally {
      setExporting(false);
    }
  };
  ...
  return (
    <div>
      ...
      {status.type === "ok" ? (
        <div className="mb-4 rounded-lg bg-green-50 px-3 py-2 text-[12px] text-green-700">{status.msg}</div>
      ) : null}
              ...
              <div className="flex flex-1 flex-col gap-1">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      type="button"
                      onClick={() => void copyForSubstack()}
                      disabled={exporting || !bodyMarkdown.trim()}
                      className="rounded-pill border border-green-600 px-2.5 py-1 text-[11.5px] font-semibold text-green-700 transition-colors hover:bg-green-50 disabled:opacity-40"
                    >
                      {exporting ? "Exporting…" : "Copy for Substack"}
                    </button>
                    <button
                      type="button"
