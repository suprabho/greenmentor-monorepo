...
export function StoryBody({ markdown }: { markdown: string }) {
  ...
  return (
    <div className="flex flex-col gap-4" style={brandVars}>
      {blocks.map((b, i) => {
        if (b.kind === "heading") {
          // Brand-green section headings, matching the GreenMentor Substack.
          return b.level === 2 ? (
            <h2 key={i} className="text-[19px] font-semibold" style={{ color: "#0B602C" }}>
              {formatInlineMarkdown(b.text)}
            </h2>
          ) : (
            <h3 key={i} className="text-[16px] font-semibold" style={{ color: "#0B602C" }}>
              {formatInlineMarkdown(b.text)}
            </h3>
          );
        }
